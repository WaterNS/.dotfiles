...
elif [ "$u" = true  ]; then updateGitRepo "BadWolf" "VIM theme" ~/.vim/bundle/badwolf --depth 1;
fi
...
elif [ "$u" = true  ]; then updateGitRepo "vim-airline" "VIM status bar plugin" ~/.vim/bundle/vim-airline --depth 1;
fi
...
elif [ "$u" = true  ]; then updateGitRepo "vim-airline-themes" "vim-airline themes" ~/.vim/bundle/vim-airline-themes --depth 1;
fi
...
elif [ "$u" = true  ]; then updateGitRepo "supertab" "VIM tab completion plugin" ~/.vim/bundle/supertab --depth 1;
fi
